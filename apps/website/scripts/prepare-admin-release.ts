...
if (build.enabled) {
  ...
  const config = z.object({ d1_databases: z.array(z.object({ binding: z.string(), database_id: z.string().optional() })), r2_buckets: z.array(z.object({ binding: z.string(), bucket_name: z.string() })) }).parse(JSON.parse(readFileSync(path, 'utf8')))
