...
- 工具页提供[封面制作器](docs/steps/11-cover.md)，默认白色背景、深色文字；左右文字共用固定无衬线字体，字重、字号、行距及横向／纵向偏移均支持滑动调整；文字支持多色渐变、描边与发光。支持完整本地及在线 Iconify 图标、图标玻璃底板、背景图片亚克力与可选透明背景，可下载 PNG／SVG／WebP。图片支持选择、粘贴和拖拽导入。后台可应用 PNG／WebP 并保存草稿；参数不持久保存。部署与维护见 [部署说明](DEPLOYMENT.md)。
...
生产登录与发布配置见 [第 9 步部署说明](DEPLOYMENT.md#第-9-步后台登录和评论)。OAuth、发布 PAT、giscus 的创建步骤均在该文档中。封面制作器已复用独立图片上传模块，操作与限制见 [第 11 步说明](docs/steps/11-cover.md)。
