...
- 工具页提供[封面制作器](docs/steps/11-cover.md)，默认白色背景、深色文字；左右文字共用固定无衬线字体，字重、字号、行距及横向／纵向偏移均支持滑动调整；文字支持多色渐变、描边与发光。支持完整本地及在线 Iconify 图标、图标玻璃底板、背景图片亚克力与可选透明背景，可下载 PNG／SVG／WebP。图片支持选择、粘贴和拖拽导入。后台可应用 PNG／WebP 并保存草稿；参数不持久保存。
- 整站表单下拉共用 Reka UI 选择组件，统一箭头、选中标记、键盘导航和弹窗内定位，预留滚动条空间避免展开时抖动。布尔设置使用开关，封面滑动条支持弹性反馈、精确数值输入、字重节点和纵向零基准。
...
封面制作器已复用独立图片上传模块，操作与限制见 [第 11 步说明](docs/steps/11-cover.md)。

## AI 摘要

站主在 `/admin/ai` 配置 Chat Completions 兼容服务的 HTTPS API 基址、模型、密钥和提示词；地址填至 `/v1` 等 API 基址，服务会追加 `/chat/completions`。不预设供应商，默认关闭。设置保存只影响后台生成及下一次网站构建，不会触发部署。
