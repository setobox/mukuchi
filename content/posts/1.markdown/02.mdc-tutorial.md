...
#footer
[查看 Meta 配置](/posts/1.markdown/_getting-started)
::
```
...
当前文章字段见[文章 Meta 示例](/posts/1.markdown/_getting-started)。如果要查询新的自定义字段，还需要扩展内容集合的 schema 和校验规则。数据绑定不等于在 Markdown 中编写完整 Vue 模板；业务逻辑应放在组件中。
...
```md
[![阅读 Markdown 教程](/images/content-example.svg)](/posts/1.markdown/01.markdown)
```
...
自定义组件需要按 Nuxt Content / Vue 的接口实现并注册。多风格提醒框、图片画廊和仓库卡片的状态见 [Markdown 扩展功能](/posts/1.markdown/03.markdown-extended)。
